...
api.cast = function(req, res, next) {
  ...
  if (!spell){
    return res.json(404, {err: 'Spell "' + req.params.spell + '" not found.'});
  }
  ...
  switch (targetType) {
    case 'task':
      if (!user.tasks[targetId]){
        return res.json(404, {err: 'Task "' + targetId + '" not found.'});
      }
      ...
      async.waterfall([
        ...
        function(group, cb) {
          ...
          if (targetType === 'party') {
            ...
            series = _.transform( g.members, function(m,v){
    ...
    api[k] = function(req, res, next) {
      res.locals.user.ops[k](req,function(err, response){
        ...
        if (err) {
          ...
          if (err.code >= 400){
            return res.json(err.code,{err:err.message});
          }
